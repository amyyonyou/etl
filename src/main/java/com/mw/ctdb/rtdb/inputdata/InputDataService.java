package com.mw.ctdb.rtdb.inputdata;
...
import com.mw.base.dev.DevService;
import com.mw.ctdb.rtdb.modifydelta.ModifyDeltaService;
import com.mw.ctdb.rtdb.sensor.Scada2015Sensor;
import com.mw.ctdb.rtdb.sensor.Scada2015SensorDao;
...
public class InputDataService {
	...
	public void saveData() throws Exception {
		// if (!devService.isLegalServer() || !devService.isProduction()) {
		// return;
		// }
	...
	public void convert(String tableName, List<Map<String, Object>> rowList) throws ParseException {
		...
		ffMap.put("tableName", tableName);
		List<Scada2015Sensor> fieldList = scada2015SensorDao.getBy(ffMap, null);
		...
		for (Map<String, Object> rowMap : rowList) {
			...
			String sql = null;
			try {
				sql = getAddSql(tableName, fieldList, rowMap);
				jtCTDB.update(sql, param);
			} catch (DuplicateKeyException e) {
				// logger.info("DuplicateKeyException-> rowMap:{}", rowMap);
				sql = getUpdSql(tableName, fieldList, rowMap);
				jtCTDB.update(sql, param);
			}
	...
	private String getAddSql(String tableName, List<Scada2015Sensor> fieldList, Map<String, Object> rowMap) {
		StringBuilder fieldNameBuilder = new StringBuilder();
		StringBuilder fieldValueBuilder = new StringBuilder();
		int i = 0;
		for (Scada2015Sensor scada2015Sensor : fieldList) {
			Object val = rowMap.get(scada2015Sensor.getFieldName());
			if (val == null) {
				continue;
			}
			...
			fieldNameBuilder.append(scada2015Sensor.getFieldName());
			fieldValueBuilder.append(":").append(scada2015Sensor.getFieldName());
		}
	...
	private String getUpdSql(String tableName, List<Scada2015Sensor> fieldList, Map<String, Object> rowMap) {
		StringBuilder fieldNameBuilder = new StringBuilder();
		int i = 0;
		for (Scada2015Sensor scada2015Sensor : fieldList) {
			Object val = rowMap.get(scada2015Sensor.getFieldName());
			...
			fieldNameBuilder.append(scada2015Sensor.getFieldName()).append("=").append(":").append(scada2015Sensor.getFieldName());
		}
	...
	private Double parseVal(String fieldName, Double val) {
		if (StringUtils.contains(fieldName, "capacity10000")) {
			if (val != null) {
				val = val / 10000;
			}
		}
